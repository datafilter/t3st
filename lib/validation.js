class ValidationError extends Error {
    // TODO: separate cause from stack.
    // constructor(cause) {
    //     super()
    //     this.cause = cause
    // }
...
const error_result = (description, error, origin) =>
    ({
        ...
        is_error: true,
        error,
        trace: error && error.stack ? error_origin(error, origin) : error_origin(new Error(), origin),
        is_validation_error: error instanceof ValidationError
    })

const error_origin = (err, origin = []) => {

    const err_stack = err.stack.split('\n').slice(1) //|| (new Error().stack)).split("\n").slice(1)
    ...
    if (typeof origin === 'undefined' || typeof err_stack === 'undefined') {
        console.log('!?ding whutnow ??')
        console.log('origin', origin)
        console.log('err_stack', err_stack)
    }
