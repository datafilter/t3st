...
const check = (...factors) => {
    ...
    let error_message = '' //todo replace with const

    if (typeof assumption === 'function') {
        error_message = evaluate(assumption, propositions)
    }
    else if (typeof assumption == 'boolean') {
        const all_true = () => factors.every(f => f === true)
        error_message = evaluate(all_true, [])
    } else throw new ValidationError(`check doesn't support (...booleans, ${typeof assumption}).` +
