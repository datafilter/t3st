...
const affirm = (...factors) => {
    if (factors.length === 0)
        throw new ValidationError('affirm expected (...values, function => boolean)')
...
// todo reuse elsewhere also? eg: within equal ,and display.js
...
const throw_unequal = (a, b) => {
    const e = new ValidationError(`Evaluation [${quote_wrap(a)}] === [${quote_wrap(b)}]`)
    if (is_plain_object(a) && is_plain_object(b)) {
        const display = require('./display')
        e.display = () => {
            display.print_table_diff(a, b)
        }
    }
    throw e
}
...
const equal = (a = equal_hint_argument(), b = equal_hint_argument()) => {
    if (a !== b) {
        const tA = typeof a
        const tE = typeof b
        const type_diff = (tA === tE) ? '' : `\n\t-> Type mismatch: equal(${tA}, ${tE}).`
        if (type_diff) {
            throw new ValidationError(`Evaluation [${quote_wrap(a)}] === [${quote_wrap(b)}]${type_diff}`)
        }
        if (tA === 'object') {
