module.exports = ({ test, throws, equal, check }) => {
    ...
    const equal_chain_tests = [
        ...
        , test("chained equal stops at first error", () => {
            ...
            check(err_third.error.message, (m) => m.includes('Evaluation [1] === [5]'))
        })
    ]
    ...
    const equal_equal_tests = [
        ...
        , throws("strict equal shows type mismatch error", () => {
            ...
            check(err.message, (m) => m.includes('Type mismatch: equal(string, number)'))
        })
        ...
        , test("Evaluation is included in error message", () => {
            check(test("err", () => equal(true, false)).error.message, (m) =>
                m.includes("Evaluation [true] === [false]"))
            check(test("err", () => equal(true, 'true')).error.message, (m) =>
                m.includes("Evaluation [true] === ['true']"))
            check(test("err", () => equal("text", `other text`)).error.message, (m) =>
                m.startsWith("Evaluation ['text'] === ['other text']"))
            check(test("err", () => equal(5, true)).error.message, (m) =>
        ...
        , test("equal nothing or undefined returns error", () => {
            ...
            check(() => nullary.error.message.includes(undefined_argument_error))
            check(() => unary.error.message.includes(undefined_argument_error))
            check(() => binary.error.message.includes(undefined_argument_error))
        })
        , test("equal value against undefined returns error", () => {
            ...
            check(() => missing_2nd.error.message.includes(undefined_argument_error))
            check(() => undefined_1st.error.message.includes(undefined_argument_error))
            check(() => undefined_2nd.error.message.includes(undefined_argument_error))
        })
        , test("error in equal is caught in test before it's passed to equal", () => {
            ...
            check(test_err.error + '', err_equal.error + '', (t, e) => t === e)
        })
    ]
