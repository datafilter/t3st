module.exports = (framework) => {
    ...
    const test_tests = [
        ...
        , test("unexpected body returns error result", () => {
            const unexpected_primitives = [
                null
                // , undefined //different result, see test "without body returns error"
                , 0
                , 1
                , 'oh, hi mark'
                , Symbol()
            ]
            const every_test_invalid = unexpected_primitives.every(p => {
                const invalid_test = test("_", p)
                return invalid_test.error.includes(`invalid test body type in test(string, ${typeof p})`)
            })
            assert(true, every_test_invalid)
        })
